...
def image_filter(image, filter_mask, numprocessors, filtered_image):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            # We start at the midpoint then go forward or backward depending on the size of the filter
            # 1D array
            if midrow == 0:
                ...
                for i in range(-midrow, midrow + 1):
                    for j in range(-midcol, midcol + 1):
                        ...
                        if (r + i >= 0 and r + i < rows) and (c + j >= 0 and c + j < cols):
                            # Multiply the filter value with the corresponding pixel value
                            filtered_value += np.uint8(image[r + i, c + j] * filter_mask[i + midrow][j + midcol])
